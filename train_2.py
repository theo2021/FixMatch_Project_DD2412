...
def train_fixmatch(model, ema, trainloader, validation_loader, augmentation, optimizer, scheduler, device, K, tb_writer):
    ...
    with tqdm(total = K) as bar:
        ...
        for i, (label_load, ulabel_loader) in enumerate(trainloader):
            ...
            if (scheduler.state == 2 or i > 0.01*K ) and (end_warmup is False): # if exited warmup phaze
                ema.register()
                end_warmup = True
...
if __name__ == "__main__":
    ...
    scheduler = cosineLRreduce(optimizer, K*10, warmup=args.warmup_scheduler/10)

    train_fixmatch(model,ema, zip(lbl_loader, ulbl_loader), v_loader, augmentation, optimizer, scheduler, device, K, tb_writer)
    tb_writer.close()

    # Save everything
    save_models([model, 'normal'], prefix='_final_' + str(datetime.datetime.now()))
    ema.apply_shadow()
    save_models([model, 'ema'], prefix='_final_' + str(datetime.datetime.now()))
